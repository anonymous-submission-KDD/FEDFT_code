...
class TransformationRecord(Record):
    ...
    def get_permutated(self, max_size=0, num=25, padding=True, padding_token=eos_token):
        ...
        for i in range(num):
            round = []
            indice = np.random.permutation(range(size))
            for j in indice:
    ...
    def op(self, df, with_original=False):
        ...
        if self.valid:
            ...
            if with_original:
                out_dg = pd.concat(
                    [pd.DataFrame(df.iloc[:, :-1].values, columns=[df.columns[:-1]], index=range(len(df))),
                     new_dg], axis=1)
...
if __name__ == '__main__':
    ...
    example_df = pd.read_hdf('./data/airfoil.hdf')
