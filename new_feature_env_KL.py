...
from Record import Record, RecordList, TransformationRecord, TransformationRecordList
from utils.datacollection.Operation import converge, sep_token, show_ops, show_ops_r
from utils.datacollection.logger import error, info
from rlac_tools import (
    test_task_new,
    downstream_task_new,
    downstream_task_by_method,
    downstream_task_by_method_std,
    downstream_task_new_score,
)
...
TASK_DICT = {'airfoil': 'reg', 'airfoil_1': 'reg', 'airfoil_2': 'reg', 'airfoil_3': 'reg', 'airfoil_4': 'reg', 'airfoil_5': 'reg', 
            ...
            'amazon_employee_100': 'cls', 'amazon_employee_101': 'cls',
             ...
             'bike_share': 'reg', 
             'german_credit': 'cls', 'german_credit_1': 'cls','german_credit_2': 'cls','german_credit_3': 'cls','german_credit_4': 'cls',
             'german_credit_5': 'cls',
             ...
             'spectf': 'cls', 
             'spectf_1': 'cls', 'spectf_2': 'cls', 'spectf_3': 'cls', 
             'spectf_a1': 'cls','spectf_a1_1': 'cls','spectf_a1_2': 'cls','spectf_a1_3': 'cls',
             'spectf_a0p5': 'cls','spectf_a0p5_1': 'cls','spectf_a0p5_2': 'cls', 'spectf_a0p5_3': 'cls',
             'svmguide3': 'cls',
             'svmguide3_1': 'cls','svmguide3_2': 'cls','svmguide3_3': 'cls','svmguide3_4': 'cls','svmguide3_5': 'cls',
             'svmguide3_a1': 'cls',
             ...
             'credit_default_a0p5': 'cls',
             'credit_default_a0p5_1': 'cls','credit_default_a0p5_2': 'cls','credit_default_a0p5_3': 'cls','credit_default_a0p5_4': 'cls','credit_default_a0p5_5': 'cls', 
...
class TransformationFeatureEvaluator(Evaluator):
    def __init__(self, task: str, from_scratch: bool = False, all: bool = True,
                 rec_length_strategy: Optional[str] = 'sigma', sigma: Optional[int] = 4,  #先调整成all试试
                 percentage: Optional[float] = 0.99, force: Optional[bool] = False, limit=1000): #这里limit先修改一下，之后弄回来，现在只是跑通
        super().__init__(task)
        ...
        self.overall_processed_seq, self.overall_performance, ds_size, self.contain_best = \
            self._get_data_from_local(all=all,
                                      ...
                                      force=force)
        info(f'generated overall_performance lentgh:{len(self.overall_performance)}')
        self.ds_size = int(ds_size) + 1
        info(f'self.ds_size = {self.ds_size}')
        ...
        if os.path.exists(suppose_file_cache_name) and not from_scratch:
            ...
            info(f"top_performance:{top_performance.shape}") #top_performance:torch.Size([200, 1]); [limit,1]
            info(f"indices:{indices.shape}")
            ...
            for seq, per in tqdm.tqdm(zip(top_seq, top_performance), desc='processed seq processing'):
                ...
                try:
                    ...
                    continue
            info(f'actual valid records:{len(self.records)}')
    ...
    def get_performance(self, data=None,return_sorted_features=False):
        if data is None:
            data = self.original
        return downstream_task_new(data, self.task_type,return_sorted_features=return_sorted_features)
    
    def get_performance_score(self, data=None,return_sorted_features=False):
        if data is None:
            data = self.original
        return downstream_task_new_score(data, self.task_type,return_sorted_features=return_sorted_features)
 
    def get_performance_allmetric(self, data=None):
        if data is None:
            data = self.original
        return test_task_new(data, self.task_type)
    ...
    def get_performance_via_method_std(self, data=None, method='RF'):
        if data is None:
            data = self.original
        return downstream_task_by_method(data, self.task_type, method)
    ...
    def _generate_data_via_record(self, record: TransformationRecord):
        if record.is_valid():
            ...
            return pd.concat([self.original, gen], axis=1)
        else:
            return None
    ...
    def _get_data_from_local(self, file_base_path='/FG-FL/utils/datacollection/tmp', all=True,
        ...
        if rec_length_strategy == 'all':
            ...
            choosed_seq = overall_processed_seq
            ...
            best_removed = max(outlier_x) if outlier_x else 'N/A'
            best_original = max(overall_performance) if overall_performance else 'N/A'
            info(f'using {sigma}-sigma to remove outlier')
            if best_removed:
                info(f'the best removed is {best_removed}, the original_best is {best_original}') 
            else:
                info('no outlier found')
            info(f'selected length is {max_size}')

            max_length = max_size

        elif rec_length_strategy == 'length':
            max_size = pd.DataFrame(size).describe(percentiles=[percentage]).values[-2].item()
        ...
        info(f' {len(choosed_seq)} being kept, max length {max_size}')
...
class GlobalFeatureEvaluator(Evaluator):

    def __init__(self, task_name: str, merged_records: list):

        self.task_name = task_name
        super().__init__(task_name)

        max_seq_length = max(len(record.operation) for record in merged_records) if merged_records else 0
        self.ds_size = max_seq_length + 1

        self.records = TransformationRecordList(self.ds_size)
        self.records.r_list = merged_records  


        self._init_core_properties(merged_records)

    def _init_core_properties(self, records):

        self.best_grfg = max(record.performance for record in records) if records else 0

        self.contain_best = True  
        self.overall_processed_seq = [record.operation for record in records]
        self.overall_performance = [record.performance for record in records]
    ...
    def get_performance_allmetric(self, data=None):
        if data is None:
            data = self.original
        #return_sorted_features=return_sorted_features
        return test_task_new(data, self.task_type)
