...
class Encoder(nn.Module):
    ...
    def infer(self, x, predict_lambda, direction='-'):                              
        ...
        encoder_outputs, encoder_hidden, seq_emb, predict_value, _, _ = self(x) 
...
class RNNEncoder(Encoder):
    ...
    def forward(self, x):
        ...
        h = torch.mean(out, dim=1)                  # [B, H]
        mu     = self.fc_mu(h)                      # [B, H] ——> [B, latent_dim]
        logvar = self.fc_logvar(h)                  # [B, H] ——> [B, latent_dim]
