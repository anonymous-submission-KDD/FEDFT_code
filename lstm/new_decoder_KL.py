...
class Decoder(nn.Module):
    ...
    def beam_forward(self, x, encoder_hidden=None, encoder_outputs=None, beams=5):
        ...
        encoder_hidden = self._prepare_encoder_hidden_for_beam_search(encoder_hidden, beams) #(num_layers, B, H)->(num_layers, B*beams, H)
        encoder_outputs = self._prepare_encoder_output_for_beam_search(encoder_outputs, beams) #(B, S, H)->(B*beams, S, H)
        x, batch_size, length = self._validate_args(x, encoder_hidden,
                                                    encoder_outputs,
                                                    self.gpu)  
