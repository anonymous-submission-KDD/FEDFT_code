...
sys.path.insert(0, os.path.expanduser('~/jupyter_base/RL-TKDE/'))

# 动态获取项目根目录（FG-FL 的绝对路径）
current_script_path = os.path.abspath(__file__)  # 当前脚本的绝对路径
baseline_dir = os.path.dirname(current_script_path)  # baseline目录
datacollection_dir = os.path.dirname(baseline_dir)     # datacollection目录
utils_dir = os.path.dirname(datacollection_dir)        # utils目录
project_root = os.path.dirname(utils_dir)              # FG-FL目录（项目根目录）
...
#from datacollection.DQN import DQN1, DQN2, DDQN1, DDQN2, DuelingDQN1, DuelingDDQN2, DuelingDQN2, DuelingDDQN1
#from datacollection.tools import *
#from datacollection.task_mapping import task_dict, task_type, base_path, task_measure, state_rep, support_rl_method
import argparse
...
def model_train(param, nni):
    ...
    # 对测试集也进行转换
    #Dg_.columns = [str(i) for i in Dg_.columns]
...
if __name__ == '__main__':
    for name in [
        ...
        try:
            ...
            info('Total training time for dataset {}: {:.1f}s'.format(params['file_name'], total_training_time))
